...
## (6) Evaluation of all differences

This map shows a summary of all average difference values in Chlorophyll for selected year window and number of restoration project matches for a selected restoration project type.  The restoration project grouping (simple: top, complex: type) can also be toggled.  The points are displayed by size to show magnitude of the change and color to show direction of the change (red is increase, green is decrease). These are the same values that are shown in tab 5 but mapped to all stations for any variable combination from the selection tabs.
...
column(12, 
       ...
       column(4, 
              renderUI({
                ...
                selectInput('prjtyp', 'Select restoration type:', choices = sels)
...
renderPlot({
  ...
  datdfeval <- datdf %>% 
    filter(resgrp %in% prjtyp) %>% 
    left_join(wqstat, by = 'stat') %>% 
    rename(`Chlorophyll\nchange` = avedf)
  
  # map
  p1 <- pbase + 
    geom_point(data = datdfeval, aes(x = lon, y = lat, fill = `Chlorophyll\nchange`, size = abs(`Chlorophyll\nchange`)), pch = 21, colour = 'black') +
