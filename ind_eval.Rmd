...
For each of `r renderText({input$mtch})` restoration project types, the water quality observations that occurred `r renderText({input$yrdf})` years before and after the completion of the project were selected. The date of completion for each project is shown with teh thick 
...
Take the average of the averaged difference across the `r renderText({input$mtch})` projects per project type.
