...
```{r globals, eval = T, message = F, warning = F}
...
# run
outall3 <- foreach(i = 1:n, .packages = c('tidyr', 'dplyr', 'raster', 'sp', 'rgeos', 'tibble', 'rgdal', 'purrr', 'geosphere')) %dopar% {
...
save(outall3, file = 'data/outall3.RData', compress = 'xz')
```
...
## Random locations, similar dates, random project types (n = 50)
```{r eval = F}
# setup parallel
ncores <- detectCores() - 1
registerDoParallel(cores = ncores)
