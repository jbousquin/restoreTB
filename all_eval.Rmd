...
column(width = 12, 
  ...
  column(width = 6, 
    ...
    renderUI({
      ...
      selectInput("mtch1", label = 'Select number of restoration matches:', choices = mtch, selected = max(mtch))
  ...
  column(width = 6, 
    ...
    #     unique
    # 
    #   selectInput("mtch2", label = 'Select second match:', choices = mtch, selected = max(mtch))
    #   
    # })
...
plosta <- reactive({
  ...
  plosta <- grdsres %>% 
    filter(resgrp == resgrp2) %>% 
    filter(yrs == yrgrp) %>% 
    filter(mtch %in% as.numeric(mtch1)) %>% 
    mutate(
...
renderPlot({
  ...
  p <- ggplot(plosta, aes(x = yrdf, y = chg, group = salev, colour = salev)) +
    geom_line() +
    geom_point(size = 3) +
    # stat_smooth(method = 'lm', se = F) +
    facet_grid(chlev ~ project) +
    theme_bw(base_size = 16) +
    theme(
      strip.background = element_blank(), 
      legend.position = 'top'
    ) + 
    scale_colour_discrete("Salinity zone") +
