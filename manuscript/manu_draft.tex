...
to overall Bay improvements, the cumulative and contributory effects
relative to large-scale management efforts are not well understood.
Understanding the impacts within relevant spatial boundaries and how
these projects have jointly contributed to changes over time will
provide an improved understanding of the link between Bay improvements
...
of developing the analysis framework. A simple matching of restoration
projects and water quality data was not possible because each dataset
described events or sampling activities with unique dates and locations.
To address this challenge, observations in each dataset were spatially
and temporally matched following a specific rulset that was developed to
maximize the potential of identifying a unique effect of the restoration
projects on changes in water quality. The matching between the two
datasets began with a spatial join where the Euclidean distances between
each water quality station and each restoration project were quantified.
The distances from each station to each project site were ranked from
closest to farthest. The temporal matches between water quality stations
and restoration projects were obtained by subsetting the water quality
data within a window before and after the completion date of each
spatially-matched restoration project. The combined spatial and temporal
matching between the water quality sites and restoration proejcts were
used as input for developing the probabilistic models.
