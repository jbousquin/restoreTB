...
strictly realted to restoratoin project effects. \Cref{fig:prjsig}b
...
\%latex.default(totab{[}, -c(1, 2){]}, file = ``'', caption.loc =
``top'', caption = cap.val, rowlabel = ``Bay segment'', rowname =
totab\(locs, rgroup = unique(totab\)yrdf\_mtch), n.rgroup = c(4, 4, 4,
4), cgroup = c(``'', ``Restoration projects''), n.cgroup = c(1, 5),
label = ``tab:prjsigseg'', col.just = rep(``l'', ncol(totab)),
