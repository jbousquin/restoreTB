...
p2 <- ggplot(tmdf1, aes(x = name, y = value, group = name)) +
  ...
  geom_text(data = filter(tmdf3, !name %in% 'Water quality'), aes(x = name, y = Before),
            label = 'Before', vjust = -0.75, size = 4, family = 'serif') +
  geom_text(data = filter(tmdf3, !name %in% 'Water quality'), aes(x = name, y = After),
            label = 'After', vjust = -0.75, size = 4, family = 'serif') +
...
$$
where $\delta WQ$ was the difference between the after and before changes for each of $n$ spatially  matched restoration projects.  For each $i$ of $n$ projects ($proj$), the average water quality ($\hat{wq}$) within the window ($win$) either before ($proj_{i, dt} - win$) or after ($win + proj_{i, dt}$) the completion date ($dt$) for project $i$ was summed.  The summation of water quality before after each project was then divided by the total number of $n$ matched projects, muliplied the distance of the projects from a water quality station ($dist_{i \in n}$). This created  weighted average of the before/after effects of each project that was inversely related to the distance from a watwer quality station. The total change in water quaility was simply the difference in weighted averages.
